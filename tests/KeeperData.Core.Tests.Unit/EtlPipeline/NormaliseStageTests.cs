...
public class NormaliseStageTests
{
    ...
    /// finishes, so the two differ on any file that takes more than a second - which the sample bulk
    /// does. Requiring them to match would reject every bulk cut.</summary>
    [Fact]
    public async Task NormaliseStage_ReadsAnHcdtFile_WhoseHeaderAndTrailerStampsDiffer()
    {
        const string rawFileKey = "raw/sam_cph_holdings/LITP_SAMCPHHOLDING_20260101.csv";
        const string destinationKey = "sam_cph_holdings/LITP_SAMCPHHOLDING_20260101.parquet";
    ...
    /// </summary>
    private async Task<List<NormalisedFileSet>> RunStageAsync(RawFileSet input, NormaliseStage? stage = null)
    {
        var asyncInput = AsyncEnumerableHelper(input);
        var outputStream = (stage ?? _sut).RunAsync(asyncInput, _pipelineContext, CancellationToken.None);
