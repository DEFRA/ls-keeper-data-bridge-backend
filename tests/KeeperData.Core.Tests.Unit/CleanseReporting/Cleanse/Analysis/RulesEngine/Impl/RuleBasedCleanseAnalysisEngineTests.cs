...
namespace KeeperData.Core.Tests.Unit.CleanseReporting.Cleanse.Analysis.RulesEngine.Impl;

/// <summary>
/// Mirrors <c>CleanseAnalysisEngineTests</c> against the rule based engine, so that both
/// engines are held to the same observable behaviour.
/// </summary>
public class RuleBasedCleanseAnalysisEngineTests
{
    ...
    private readonly RuleBasedCleanseAnalysisEngine _sut;
    ...
    public async Task Execute_CtsEmailsAllInSam_ShouldRaiseRule6()
    {
        SetupCtsHoldings("UK-12/345/6003");
        SetupSamHoldings();
        SetupMatchingPair("12/345/6003", ctsEmails: ["a@b.com", "x@x.com"], samEmails: ["a@b.com", "extra@sam.com"], ctsPhones: ["01234"], samPhones: ["01234"]);
    ...
    [Fact]
    public async Task Execute_InvalidCountyCode_ShouldSkipSamRecord()
    {
        SetupCtsHoldings();
        SetupSamHoldings("99/345/6001");
    ...
    [Fact]
    public async Task Execute_UnparseableIdentifiers_ShouldBeSkipped()
    {
        SetupCtsHoldings("not-a-lid");
        SetupSamHoldings("not-a-cph");
    ...
    [Fact]
    public async Task Execute_ShouldReturnMetricsCountingRecordedIssues()
    {
        SetupCtsHoldings("UK-12/345/6001");
        SetupSamHoldings();
        _dataServiceMock.Setup(s => s.GetSamCphHolding(CphFor("12/345/6001")))
        ...
        var metrics = await _sut.ExecuteAsync(OperationId, CancellationToken.None);

        metrics.IssuesFound.Should().BeGreaterThan(0);
    }

    [Fact]
    public async Task Execute_WhenRecordingMadeNoChange_ShouldNotCountTheIssue()
    {
        _issueServiceMock.Setup(s => s.RecordIssueAsync(It.IsAny<RecordIssueCommand>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(IssueRecordResult.NoChange);
